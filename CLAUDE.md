...
- **다음 차례**: **W2(T2.x)** — 크리티컬 패스 T2.1(puppeteer/chromium 설치) → T2.3(PDF 라우트), T2.2(폰트)·T2.4(revalidate)·T2.5(robots) 병행 가능.
...
> 도메인 타입(`Quote`, `QuoteItem`, `QuoteTotals`, `QuoteStatus`)은 W1 의 T1.2-types 에서 `types/index.ts` 에 정의. 현재 `types/index.ts` 에는 `ThemeMode` 만 있다.
...
- **PDF 생성 (W2)** — `@sparticuz/chromium` + `puppeteer-core` 조합. Vercel Function 메모리 1024MB 이상 권장. 패키지는 W2 작업 시 설치한다.
...
app/q/[slug]/quote-data.tsx    # ✅ T1.5 — "use cache" 데이터 컴포넌트 (getQuoteBySlug→notFound, items·totals, 임시 뷰)
app/q/[slug]/quote-skeleton.tsx# ✅ T1.5 — Suspense fallback 로딩 스켈레톤
proxy.ts                       # ✅ T1.5 — /q/* 응답에 X-Robots-Tag noindex 강제 (구 middleware.ts)
lib/quotes.ts                  # ✅ T1.2/T1.3 — Notion 페치 (server-only + databases.retrieve→dataSources.query)
app/q/[slug]/pdf/route.ts      # ⬜ T2.3 — 헤드리스 Chromium PDF 생성
app/api/revalidate/route.ts    # ⬜ T2.4 — on-demand revalidate webhook (Bearer 인증)
```

> ⚠️ `quote-data.tsx` 의 렌더 부분은 **T1.5 임시 뷰**다. T1.6 에서 `components/quote-view.tsx`(`<QuoteView quote items totals itemsWarning isExpired>`) 로 교체. 교체 지점: `quote-data.tsx` 의 `// ─── 임시 뷰 ───` 블록. `isExpired` 는 T1.7 의 `isQuoteExpired` 추가 후 연결.
